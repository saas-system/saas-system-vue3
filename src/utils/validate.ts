...
export function buildValidatorData({ name, message, title, trigger = 'blur' }: buildValidatorParams): FormItemRule {
    ...
    if (name == 'upload') {
        return {
            required: true,
            message: message ? message : i18n.global.t('Please upload field', { field: title }),
            trigger: trigger,
        }
    }
    ...
    if (name == 'select') {
        return {
            required: true,
            message: message ? message : i18n.global.t('Please select field', { field: title }),
            trigger: trigger,
        }
    }

    // 常见类型
    const validatorType = ['number', 'integer', 'float', 'date', 'url', 'email', 'upload','select']
